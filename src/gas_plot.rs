...
use super::instruments::{Gas, GasType};
use super::myapp::get_max_y;
use super::myapp::get_min_y;
...
//
//     let gas_data = &cycle.gas_data;
//     let ymin = get_min_y(cycle);
//     let ymax = get_max_y(cycle);
//     // let ymin = cycle
//     //     .gas_v
//     //     .iter()
//     //     .cloned()
...
//     let data: Vec<(f64, f64, GasType)> = time_data
//         .iter()
//         .zip(gas_data.iter())
//         .flat_map(|(&t, gas)| match gas {
//             Gas::CH4(values) => values
//                 .iter()
//                 .map(move |&g| (t, g, GasType::CH4))
//                 .collect::<Vec<_>>(),
//             Gas::CO2(values) => values
//                 .iter()
//                 .map(move |&g| (t, g, GasType::CO2))
//                 .collect::<Vec<_>>(),
//             Gas::H2O(values) => values
//                 .iter()
//                 .map(move |&g| (t, g, GasType::H2O))
//                 .collect::<Vec<_>>(),
//             Gas::N2O(values) => values
//                 .iter()
//                 .map(move |&g| (t, g, GasType::N2O))
//                 .collect::<Vec<_>>(),
//         })
//         .collect();
